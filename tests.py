...
try:
    ...
    log(
        "Import",
        True,
        f"Successfully imported {PACKAGE_NAME} version {getattr(torchfbm, '__version__', 'unknown')}",
    )

except ImportError as e:
    log(
        "Import", False, f"Failed to import. Did you run 'pip install -e .'? Error: {e}"
    )
...
try:
    ...
    log(
        ...
        path_fast.shape == (4, 1001),
        "Shape (Batch, Time+1) correct",
    )
...
try:
    ...
    log(
        "Streaming Step",
        val1.numel() == 1 and val2.numel() == 1,
        "Generated scalar steps",
    )
...
bridge = fractional_brownian_bridge(
    n=200, H=0.3, start_val=start_val, end_val=end_val, device=device
)
...
log(
    "Noisy Linear (Train)",
    not torch.allclose(y_train_1, y_train_2),
    "Noise active (stochastic output)",
)
...
log(
    "Noisy Linear (Eval)",
    torch.allclose(y_eval_1, y_eval_2),
    "Noise frozen (deterministic output)",
)

# Positional Embeddings
emb = FractionalPositionalEmbedding(max_len=50, d_model=64, H_range=(0.1, 0.9)).to(
    device
)
...
action_noise_torch = FBMActionNoise(
    mean=0, sigma=0.1, H=0.3, size=(2,), device=device.type
)
n_torch = action_noise_torch()
log(
    "RL Noise (PyTorch)",
    isinstance(n_torch, torch.Tensor),
    "Returns PyTorch tensor by default",
)

# RL Action Noise (NumPy option - skip if NumPy unavailable)
try:
    action_noise_numpy = FBMActionNoise(
        mean=0, sigma=0.1, H=0.3, size=(2,), device=device.type, return_numpy=True
    )
    n_numpy = action_noise_numpy()
    log(
        "RL Noise (NumPy)",
        isinstance(n_numpy, np.ndarray),
        "Returns NumPy array when requested",
    )
except ImportError:
...
model = NeuralFSDE(
    state_size=1, drift_net=MockDrift(), diffusion_net=MockDiff(), learnable_H=True
).to(device)
