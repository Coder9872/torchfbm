...
class FBMNoisyLinear(nn.Module):
    def __init__(self, in_features, out_features, H=0.5, sigma_init=0.5, buffer_size=1000, method='davies_harte', device='cpu', dtype=torch.float32, seed: int | None = None):
        super().__init__()
...
class FractionalPositionalEmbedding(nn.Module):
    ...
    """
    def __init__(self, max_len, d_model, H_range=(0.1, 0.9), method='davies_harte', device='cpu', dtype=torch.float32, seed: int | None = None):
        super().__init__()
