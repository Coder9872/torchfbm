...
def generate_cholesky(n: int, H: float, size: tuple = (1,), device='cpu', dtype: torch.dtype = torch.float32, seed: int | None = None) -> torch.Tensor:
    ...
    noise = torch.randn(*size, n, device=device, dtype=dtype)
    return torch.matmul(noise, L.t())

def generate_davies_harte(n: int, H: float, size: tuple = (1,), device='cpu', dtype: torch.dtype = torch.float32, seed: int | None = None) -> torch.Tensor:
    device = torch.device(device)
    if seed is not None:
        torch.manual_seed(seed)
    gamma = _autocovariance(H, n, device, dtype)
    ...
    lambdas = torch.clamp(lambdas, min=torch.finfo(dtype).tiny)  # tiny floor
    rng_real = torch.randn(*size, M, device=device, dtype=dtype)
    rng_imag = torch.randn(*size, M, device=device, dtype=dtype)
...
def fbm(n: int, H: float, size: tuple = (1,), method='davies_harte', device='cpu', dtype: torch.dtype = torch.float32, seed: int | None = None):
