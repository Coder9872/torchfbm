...
class FBMNoisyLinear(nn.Module):
    def __init__(
        ...
        self.weight_mu = nn.Parameter(
            torch.empty(
                out_features, in_features, device=self._device, dtype=self._dtype
            )
        )
        ...
        self.register_buffer(
            "noise_stream_in",
            torch.empty(
                in_features, buffer_size, device=self._device, dtype=self._dtype
            ),
        )
        self.register_buffer(
            "noise_stream_out",
            torch.empty(
                out_features, buffer_size, device=self._device, dtype=self._dtype
            ),
        )
